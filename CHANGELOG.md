...
### Added
- Configured Progressive Web App (PWA) installability.
- Created `manifest.webmanifest` defining application name, start URL, portrait orientation, BMR background variables, and `display: "standalone"` settings.
- Wrote basic service worker `sw.js` enabling offline app checks and satisfying browser PWA install triggers.
- Registered PWA configuration assets inside `angular.json`.
- Registered and initialized client service worker listener hooks inside `index.html`.
